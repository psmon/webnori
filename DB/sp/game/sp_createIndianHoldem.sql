DROP PROCEDURE IF EXISTS sp_CreateIndianHoldem;
DELIMITER $$
CREATE PROCEDURE sp_CreateIndianHoldem (
  IN mainType INT,
...
BEGIN    
  ...
  select Count(*) into _publishCnt from tblIndianHoldemList where creatorID = _creatorID and tblIndianHoldemList.gameState = 0 and tblIndianHoldemList.mainType=0;  
  ...
  BEGIN
    ...
    if mainType=0 then
      update tblbankinfo set gamePoint = _gamePoint,coinCount=_coinCount where memberID = _creatorID;  
    end if;
    
    select nick,picurl into _creatorNick,_creatorAvatar from tblmember where memberID = _creatorID;    
  
    insert into tblIndianHoldemList( 
    ...
    values(
      NULL,mainType,-1,gameTitle,_creatorID,_creatorNick,gameData,gameCount,0,gameMoneyAmount,_createtime,_creatorAvatar,_lockPoint    
    );    
    COMMIT;    
  END;
  END IF;
